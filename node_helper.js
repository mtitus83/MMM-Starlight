...
// Flag to track if a midnight update is in progress
let isPerformingMidnightUpdate = false;

// Attempt to import node-schedule, logging success or failure
let schedule;
try {
...
// Function to parse and potentially encode certain patterns in the horoscope text
...
// Create and export the NodeHelper module
...
fetchHoroscope: async function (period, zodiacSign) {
    try {
        let requestUrl;
        // Construct the appropriate URL based on the requested period
        switch (period) {
            case "daily":
...
handleInit: function(payload) {
    if (payload && payload.config) {
        ...
        // Initialize the cache and schedule updates
...
scheduleMidnightUpdate: function() {
    ...
    // Schedule the midnight update job
